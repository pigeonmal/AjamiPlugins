...
open class AjamiStreamingProvider : TmdbProvider() { 
  ...
  val adultQuery =
            if (settingsForProvider.enableAdult) "&include_adult=false" else "&without_keywords=190370|13059|226161|195669"
  ...
  private fun Media.toSearchResponse(type: String? = null): SearchResponse? {
    ...
    override suspend fun search(query: String): List<SearchResponse>? {
        return app.get("$tmdbAPI/search/multi?api_key=$apiKey&language=fr-FR&query=$query&page=1&include_adult=${settingsForProvider.enableAdult}")
            .parsedSafe<Results>()?.results?.mapNotNull { media ->
